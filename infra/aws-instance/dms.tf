...
resource "aws_dms_endpoint" "mongo_source" {
  ...
  // because cannot work with mongodb+srv, we need to extract only one server name and use it
  // we also will use the private endpoint to avoid the public internet
  // regexp help to extract first server name and port
  server_name                 = regex("mongodb://([^:]+):", mongodbatlas_advanced_cluster.atlas-cluster.connection_strings.0.private_endpoint.0.connection_string)[0]
  port                        = regex("mongodb://([^:]+):([0-9]+),", mongodbatlas_advanced_cluster.atlas-cluster.connection_strings.0.private_endpoint.0.connection_string)[1]
  database_name               = "test"
  ssl_mode                    = "require"
  extra_connection_attributes = "authMechanism=SCRAM_SHA_1;authSource=admin;authType=PASSWORD;extractDocId=false;nestingLevel=NONE;"
...
// CrateDB DMS connection results in an error
//  > Test Endpoint failed: Application-Status: 1020912, Application-Message: postgres_set_encoding(...)
//    failed in fetching from statement 'select setting from pg_settings where name = 'server_encoding'' No data returned.
// This is issue on CrateDB site.
